...
selected_specialty <- "007 Allergy and Immunology"
clean_selected_specialty <- str_replace_all(selected_specialty, " ", "_")
statewide_count_allergy <- ALL_statewide_specialty_count %>%
...
ggmap(co_map, extent = "device") +
  # geom_density2d(data = allergy_007,
  #                aes(x = lon, y = lat),
  #                size = 0.3) +
  # stat_density2d(data = allergy_007,
  ...
  xlab(" ") +
  ylab(NULL) +
  ggtitle('2018 "007 Allergy and Immunology" Providers') +
  geom_text(data = statewide_count_allergy, aes(x = -107, y = 41, label = str_c(count, " Providers"))) +
  labs(caption = "\n\n  Graphic by Colorado Health Insurance Insider | @lukkyjay                                                                                          Source: SERFF") +
  theme_provider_maps +
  ggsave(filename = "Plots/allergy_007.png", width = 9, height = 5, dpi = 1200)
