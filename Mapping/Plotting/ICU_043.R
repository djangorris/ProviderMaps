# REPLACE:
# ICU_043 (target geocoded variable name)
# 043 Critical Care Services - Intensive Care Units (ICU) (copy/paste from TOTAL_Specialty_Count spreadsheet on Desktop)

# GET COUNT FOR EACH CARRIER
selected_specialty <- "043 Critical Care Services - Intensive Care Units (ICU)"
clean_selected_specialty <- str_replace_all(selected_specialty, " ", "_")
statewide_count_ICU_043 <- ALL_statewide_specialty_count %>%
...
ggmap(co_map, extent = "device") +
  ...
  geom_text(data = statewide_count_ICU_043, aes(x = -107, y = 41, label = str_c(count, " Providers"))) +
  labs(caption = "\n\n  Graphic by Colorado Health Insurance Insider | @lukkyjay                                                                                          Source: SERFF") +
  theme_provider_maps +
  ggsave(filename = "Plots/ICU_043.png", width = 9, height = 5, dpi = 1200)
