# REPLACE:
# urology_033 (target geocoded variable name)
# 033 Urology (copy/paste from TOTAL_Specialty_Count spreadsheet on Desktop)

# GET COUNT FOR EACH CARRIER
selected_specialty <- "033 Urology"
clean_selected_specialty <- str_replace_all(selected_specialty, " ", "_")
statewide_count_urology_033 <- ALL_statewide_specialty_count %>%
...
ggmap(co_map, extent = "device") +
  ...
  geom_text(data = statewide_count_urology_033, aes(x = -107, y = 41, label = str_c(count, " Providers"))) +
  labs(caption = "\n\n  Graphic by Colorado Health Insurance Insider | @lukkyjay                                                                                          Source: SERFF") +
  theme_provider_maps +
  ggsave(filename = "Plots/urology_033.png", width = 9, height = 5, dpi = 1200)
