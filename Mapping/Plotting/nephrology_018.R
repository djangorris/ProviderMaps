# REPLACE:
# nephrology_018 (target geocoded variable name)
# 018 Nephrology (copy/paste from TOTAL_Specialty_Count spreadsheet on Desktop)

# GET COUNT FOR EACH CARRIER
selected_specialty <- "018 Nephrology"
clean_selected_specialty <- str_replace_all(selected_specialty, " ", "_")
statewide_count_nephrology_018 <- ALL_statewide_specialty_count %>%
...
ggmap(co_map, extent = "device") +
  # geom_density2d(data = nephrology_018,
  #                aes(x = lon, y = lat),
  #                size = 0.3) +
  # stat_density2d(data = nephrology_018,
  ...
  geom_text(data = statewide_count_nephrology_018, aes(x = -107, y = 41, label = str_c(count, " Providers"))) +
  labs(caption = "\n\n  Graphic by Colorado Health Insurance Insider | @lukkyjay                                                                                          Source: SERFF") +
  theme_provider_maps +
  ggsave(filename = "Plots/nephrology_018.png", width = 9, height = 5, dpi = 1200)
