# REPLACE:
# family_medicine_002 (target geocoded variable name)
# 002 Family Medicine (copy/paste from TOTAL_Specialty_Count spreadsheet on Desktop)

# GET COUNT FOR EACH CARRIER
selected_specialty <- "002 Family Medicine"
clean_selected_specialty <- str_replace_all(selected_specialty, " ", "_")
statewide_count_family_medicine_002 <- ALL_statewide_specialty_count %>%
...
ggmap(co_map, extent = "device") +
  geom_density2d(data = family_medicine_002,
                 aes(x = lon, y = lat),
                 size = 0.3) +
  stat_density2d(data = family_medicine_002,
  ...
  geom_point(aes(lon, lat),
             ...
             fill = "green4",
             alpha = 0.2,
             data = family_medicine_002,
             position = position_jitter(w = 0.002, h = 0.002)) +
  facet_wrap(~Carrier, ncol = 4) +
  xlab(" ") +
  ylab(NULL) +
  ggtitle(paste0('2018 ', '"', selected_specialty, '"', ' Providers')) +
  geom_text(data = statewide_count_family_medicine_002, aes(x = -107, y = 41, label = str_c(count, " Providers"))) +
  labs(caption = "\n\n  Graphic by Colorado Health Insurance Insider | @lukkyjay                                                                                          Source: SERFF") +
  theme_provider_maps +
  ggsave(filename = "Plots/family_medicine_002.png", width = 9, height = 5, dpi = 1200)
