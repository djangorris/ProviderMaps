# REPLACE:
# geriatrics_004 (target geocoded variable name)
# 004 Geriatrics (copy/paste from TOTAL_Specialty_Count spreadsheet on Desktop)

# GET COUNT FOR EACH CARRIER
selected_specialty <- "004 Geriatrics"
clean_selected_specialty <- str_replace_all(selected_specialty, " ", "_")
statewide_count_geriatrics_004 <- ALL_statewide_specialty_count %>%
...
ggmap(co_map, extent = "device") +
  # geom_density2d(data = geriatrics_004,
  #                aes(x = lon, y = lat),
  #                size = 0.3) +
  # stat_density2d(data = geriatrics_004,
  ...
  geom_text(data = statewide_count_geriatrics_004, aes(x = -107, y = 41, label = str_c(count, " Providers"))) +
  labs(caption = "\n\n  Graphic by Colorado Health Insurance Insider | @lukkyjay                                                                                          Source: SERFF") +
  theme_provider_maps +
  ggsave(filename = "Plots/geriatrics_004.png", width = 9, height = 5, dpi = 1200)
