# REPLACE:
# rheumatology_031 (target geocoded variable name)
# 031 Rheumatology (copy/paste from TOTAL_Specialty_Count spreadsheet on Desktop)
...
statewide_count_rheumatology_031 <- ALL_statewide_specialty_count %>%
...
ggmap(co_map, extent = "device") +
  geom_point(aes(lon, lat),
             ...
             fill = "green4",
             alpha = 0.2,
             data = rheumatology_031,
             position = position_jitter(w = 0.002, h = 0.002)) +
  facet_wrap(~Carrier, ncol = 4) +
  xlab(" ") +
  ylab(NULL) +
  ggtitle(paste0('2018 ', '"', selected_specialty, '"', ' Providers')) +
  geom_text(data = statewide_count_rheumatology_031, aes(x = -107, y = 41, label = str_c(count, " Providers"))) +
  labs(caption = "\n\n  Graphic by Colorado Health Insurance Insider | @lukkyjay                                                                                          Source: SERFF") +
  theme_provider_maps +
  ggsave(filename = "Plots/rheumatology_031.png", width = 9, height = 5, dpi = 1200)
