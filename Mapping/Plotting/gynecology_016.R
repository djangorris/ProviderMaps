# REPLACE:
# gynecology_016 (target geocoded variable name)
# 016 Gynecology (OB/GYN) (copy/paste from TOTAL_Specialty_Count spreadsheet on Desktop)

# GET COUNT FOR EACH CARRIER
selected_specialty <- "016 Gynecology (OB/GYN)"
clean_selected_specialty <- str_replace_all(selected_specialty, " ", "_")
statewide_count_gynecology_016 <- ALL_statewide_specialty_count %>%
...
ggmap(co_map, extent = "device") +
  # geom_density2d(data = gynecology_016,
  #                aes(x = lon, y = lat),
  #                size = 0.3) +
  # stat_density2d(data = gynecology_016,
  ...
  geom_text(data = statewide_count_gynecology_016, aes(x = -107, y = 41, label = str_c(count, " Providers"))) +
  labs(caption = "\n\n  Graphic by Colorado Health Insurance Insider | @lukkyjay                                                                                          Source: SERFF") +
  theme_provider_maps +
  ggsave(filename = "Plots/gynecology_016.png", width = 9, height = 5, dpi = 1200)
