# REPLACE:
# neurology_019 (target geocoded variable name)
# 019 Neurology (copy/paste from TOTAL_Specialty_Count spreadsheet on Desktop)

# GET COUNT FOR EACH CARRIER
selected_specialty <- "019 Neurology"
clean_selected_specialty <- str_replace_all(selected_specialty, " ", "_")
statewide_count_neurology_019 <- ALL_statewide_specialty_count %>%
...
ggmap(co_map, extent = "device") +
  # geom_density2d(data = neurology_019,
  #                aes(x = lon, y = lat),
  #                size = 0.3) +
  # stat_density2d(data = neurology_019,
  ...
  geom_text(data = statewide_count_neurology_019, aes(x = -107, y = 41, label = str_c(count, " Providers"))) +
  labs(caption = "\n\n  Graphic by Colorado Health Insurance Insider | @lukkyjay                                                                                          Source: SERFF") +
  theme_provider_maps +
  ggsave(filename = "Plots/neurology_019.png", width = 9, height = 5, dpi = 1200)
