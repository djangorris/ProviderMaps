...
RMHP_dental_providers <- filter(providers, 
                                 Specialty %in% c("Dental - General", 
                                                  "Dental - Periodontist", 
                                                  "Dental - Orthodontist",
                                                  "Dental - Endodontist"))
RMHP_medical_providers <- filter(providers,
                                  !Specialty %in% c("Dental - General",
                                                    "Dental - Periodontist",
                                                    "Dental - Orthodontist",
                                                    "Dental - Endodontist"))
# COUNT THE NUMBER OF PROVIDERS IN EACH CATEGORY STATEWIDE
RMHP_statewide_specialty_count <- RMHP_medical_providers %>% 
  group_by(Carrier, Specialty) %>% 
  summarise(count = n()) %>%
  arrange(-count)
# COUNT THE NUMBER OF PROVIDERS IN EACH CATEGORY IN EACH COUNTY
RMHP_county_specialty_count <- RMHP_medical_providers %>% 
  group_by(Carrier, Specialty, County) %>% 
  summarise(nproviders = n()) %>%
  arrange(-nproviders)
# ADD CONCATENATED LOCATIONS COLUMN
RMHP_medical_providers$locations <- paste0(RMHP_medical_providers$Street, ", ", RMHP_medical_providers$City, ", ", RMHP_medical_providers$State, ", ", RMHP_medical_providers$Zip)