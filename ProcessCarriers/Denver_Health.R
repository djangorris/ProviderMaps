...
DENVER_HEALTH_dental_providers <- filter(providers, 
                                 Specialty %in% c("Dental - General", 
                                                  "Dental - Periodontist", 
                                                  "Dental - Orthodontist",
                                                  "Dental - Endodontist"))
DENVER_HEALTH_medical_providers <- filter(providers,
                                  !Specialty %in% c("Dental - General",
                                                    "Dental - Periodontist",
                                                    "Dental - Orthodontist",
                                                    "Dental - Endodontist"))
# COUNT THE NUMBER OF PROVIDERS IN EACH CATEGORY STATEWIDE
DENVER_HEALTH_statewide_specialty_count <- DENVER_HEALTH_medical_providers %>% 
  group_by(Carrier, Specialty) %>% 
  summarise(count = n()) %>%
  arrange(-count)
# COUNT THE NUMBER OF PROVIDERS IN EACH CATEGORY IN EACH COUNTY
DENVER_HEALTH_county_specialty_count <- DENVER_HEALTH_medical_providers %>% 
  group_by(Carrier, Specialty, County) %>% 
  summarise(nproviders = n()) %>%
  arrange(-nproviders)
# ADD CONCATENATED LOCATIONS COLUMN
DENVER_HEALTH_medical_providers$locations <- paste0(DENVER_HEALTH_medical_providers$Street, ", ", DENVER_HEALTH_medical_providers$City, ", ", DENVER_HEALTH_medical_providers$State, ", ", DENVER_HEALTH_medical_providers$Zip)